...
This section is the only active backlog for the prototype. It does not create a
fifth active-truth document. A backlog status describes readiness and blocking;
it does not by itself authorize implementation, external runtime changes,
holdout selection, experiment execution, or semantic publication.
...
- `requirement`: the controlling Framework section or normative statement;
- `priority`: `P0`, `P1`, or `P2`;
- `status`: one of the states below;
- `depends_on`: prerequisite backlog identifiers;
- `deliverables`: files or immutable artifacts the item must produce;
- `acceptance`: observable checks required before completion;
