...
H1 preparation has advanced beyond the first retained `FAIL_PLAN_CONFLICT` envelope. Synthetic conformance inputs/expected bytes, manifests, implementation guard, Java/Python candidate evidence and independent static/evidence reviews now exist. Candidate `ea15e51df0f5410bfb5532e1c255f107ccb0975b` remains non-executable and H1 remains unbound because repository macro aggregation has one unresolved contract choice: average raw ratios then quantize once (`0.783333333333` for the reference case), or average already serialized repository values (`0.783333333334`). FDP recommends raw-ratio aggregation; decision proposal: `validation/software-factory/sf-bl005/formal-holdout-scorer-001/macro-rounding-decision-proposal.md`. Human selection is required before any protocol reseal or new envelope. No H1 implementation, conformance execution, holdout access, or dispatch is authorized.
