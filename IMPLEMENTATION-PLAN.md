...
`SF-BL-005-FORMAL-HOLDOUT-SCORER-001` is selected by Human option 1. r4 failed before stages (inline JSON filename length); use file-input replan `validation/software-factory/sf-bl005/execution-envelope-formal-holdout-scorer-001-r4-replan-001.json`. Original r4 is `validation/software-factory/sf-bl005/execution-envelope-formal-holdout-scorer-001-r4.json` under the recorded Human dispatch decision. Candidate `ea15e51df0f5410bfb5532e1c255f107ccb0975b`, stage semantics, frozen inputs and limits remain r4 bindings. Run phases once, fail-stop; retain and independently review evidence. Owned outputs: replan executionRoot and `validation/software-factory/sf-bl005/formal-holdout-scorer-001/r4-execution-001/`; candidate source is immutable. H1 remains unbound pending execution review and separate candidate merge decision. Calibration, repository selection, holdout access and production readiness remain unauthorized.
