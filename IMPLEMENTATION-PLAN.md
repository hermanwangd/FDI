...
`SF-BL-005-FORMAL-HOLDOUT-SCORER-001` is Human-authorized under `validation/software-factory/sf-bl005/execution-envelope-formal-holdout-scorer-001-r4-replan-002.json`. Human option 1 additionally permits the pinned, already-exposed RealWorld fixture only for mandatory regression. Preserve both prior failures; run three phases once in the new replan executionRoot, fail-stop, retain logs/receipts/Surefire and independently review. Candidate `ea15e51df0f5410bfb5532e1c255f107ccb0975b` and frozen scorer inputs remain immutable. Owned evidence: `validation/software-factory/sf-bl005/formal-holdout-scorer-001/r4-replan-002-execution/`. Java17 and aggregate <8GB limits apply. Candidate merge, calibration, repository selection, formal holdout and production readiness remain separately gated; H1 is UNBOUND.
