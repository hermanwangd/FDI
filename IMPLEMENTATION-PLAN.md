...
`SF-BL-005-H2-DIAGNOSTIC-CLEAR-001` extends that tool under approved design `b60717f`. Owned paths are the comparator, its focused tests, this plan, Backlog/Status, and `validation/software-factory/sf-bl005/h2-diagnostic-clear-001/`. Add `BLOCKED_MATCH`, `REVIEW_REQUIRED`, `INSUFFICIENT_EVIDENCE`, or `DIAGNOSTICALLY_CLEAR` as engineering-calibration readiness while preserving `selectionAuthorized=false`, `NOT_PROVEN_INDEPENDENT`, and `SUPPLIED_EXTRACTS_ONLY`. Verify focused tests, Java17 package with Maven/fork heaps 2GB/1GB, Python, packaged CLI determinism, hashes, and independent review. Exclude H0/H1 rebinding, threshold/tokenization changes, dataset selection, formal holdout execution, H2 completion, production readiness, and parent closure.
