...
| `SF-BL-005` | `FEATURE` | `AUTH-002`, `PK-004`, `EVID-001`, `TECH-001`; source: SF-BL-002 post-acceptance analysis | Align scenario-mapping scoring units, strengthen evidence-backed METHOD realization chains, and compare baseline/improved producers on isolated unseen data under one frozen protocol. Keep TYPE limitations explicit and outputs proposal-only. | `IN_PROGRESS` | RealWorld first cross-repository attempt failed before scoring: frozen mapper rejects AUTHENTICATE; no valid outputs or quality metrics. Candidate `2f8a517`; Graphify live verification passed. Evidence: `validation/software-factory/sf-bl005/cross-repo-realworld-001/RESULTS.md`; plan: `IMPLEMENTATION-PLAN.md#current-selection`. Revised portability selection required before rerun. Prior Petclinic007 recall70%, precision84.85% remains independently verified, not generalization or parent closure. Existing PORTABILITY-002 retains its pinned scope. Formal holdout remains gated; no SF-BL-002 closure. |
...
their defined authority gates. `SF-BL-005` is selected for the in-flight minimum
CLI/module-root correction (separately pinned scope) and CROSSREPO-REALWORLD-001
first cross-repository validation toward Human-selected recall >60% and precision
>80%, with independently frozen new-repository gold and unchanged 007 algorithms, under
`IMPLEMENTATION-PLAN.md`. `SF-BL-002` remains `IN_PROGRESS`, engineering accepted
