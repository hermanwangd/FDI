...
| `SF-BL-005` | `FEATURE` | `AUTH-002`, `PK-004`, `EVID-001`, `TECH-001`; source: SF-BL-002 post-acceptance analysis | Align scenario-mapping scoring units, strengthen evidence-backed METHOD realization chains, and compare baseline/improved producers on isolated unseen data under one frozen protocol. Keep TYPE limitations explicit and outputs proposal-only. | `IN_PROGRESS` | Human selected SF-BL-005-PORTABILITY-002: nested-test CLI identity and module-root minimum correction; active plan: `IMPLEMENTATION-PLAN.md#current-selection`. Scoped feasibility evidence accepted: `validation/software-factory/sf-bl005/acceptance-feasibility-001.md`. Formal holdout/scoring and generic LMS runner remain unselected. No SF-BL-002 closure. |
...
their defined authority gates. `SF-BL-005` is selected only for the minimum CLI/module-root correction under
