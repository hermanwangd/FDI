...
| `SF-BL-002` | `BUG` | `AUTH-002`, `PK-004`, `EVID-001`, `TECH-001`; source: immutable PKB-BL-007/011 `REVISE` evidence | Correct the misleading unresolved-reference denominator, recover only mechanically provable production receiver references, and enable evidence-backed scenario mapping without treating external/test-helper calls or proposals as Product truth. | `IN_PROGRESS` | Production-reference slice PASS: 24 references recovered, production resolution 80.45% → 93.85%, external/test-helper diagnostics separated, 1,032 Java tests pass. Scenario calibration run remains open. Evidence: `validation/software-factory/sf-bl002/remediation-evidence.json`. |
