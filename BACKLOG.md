...
| `PKB-BL-009` | `FEATURE` | `PKB-REVERSE-002` | Derive reviewable Capability and Behavior Scenario proposals from structural, repository-test, and delivery evidence. | `IN_PROGRESS` | Graphify discovery verified test indexing but found a test-to-production relationship gap; current construction is `IMPLEMENTATION-PLAN.md#pkb-bl-009-java-test-behavior-extractor`. Existing defect ledger remains evaluator-only. |
