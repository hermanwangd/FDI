...
| `SF-BL-005` | `FEATURE` | `AUTH-002`, `PK-004`, `EVID-001`, `TECH-001`; source: SF-BL-002 post-acceptance analysis | Align scenario-mapping scoring units, strengthen evidence-backed METHOD realization chains, and compare baseline/improved producers on isolated unseen data under one frozen protocol. Keep TYPE limitations explicit and outputs proposal-only. | `BLOCKED_DEPENDENCY` | Registered by Human authorization; not selected for execution. Requires approved scoring semantics, cross-repository feasibility and equivalent baseline parameterization, approved holdout and sealed truth, then an FDP Plan/envelope. Scope proposal: `docs/proposals/scenario-mapping-quality-improvement.md` sections 8–10. No automatic continuation or SF-BL-002 closure. |
...
their defined authority gates. `SF-BL-002` is the only selected item and remains
`IN_PROGRESS` pending Human terminal closure after route-aware correction
acceptance. No successor is automatically selected. `SF-BL-001`, `SF-BL-003`, and `SF-BL-005` remain unselected; `SF-BL-004` is
