...
`SF-BL-005-CROSSREPO-REALWORLD-003` calibration retry under
`IMPLEMENTATION-PLAN.md`. The reviewed gate correction is durably replayed; the
new execution must use its fresh exact envelope and the unused second-run output
namespace. `SF-BL-002` remains `IN_PROGRESS`, engineering accepted
