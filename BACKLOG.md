...
| `PKB-BL-025` | `FEATURE` | `PKB-REVIEW-004` | Record version-bound human ACCEPT / EDIT / REJECT decisions. | `IN_PROGRESS` | All remaining decisions are authorized for prototype reconciliation: 6 ACCEPT and 2 REJECT in `ai-provisional-review-001.json`, bound by `ai-provisional-review-acceptance-001.json`. Execution must create proposal/review revision 2 before freezing. |
...
| `PKB-BL-006` | `FEATURE` | `PKB-SCENARIO-004` | Create an approved frozen scenario-bearing semantics revision without overwriting Petclinic. | `IN_PROGRESS` | Slice B follows the revision-2 review binding in BL-025; expected frozen set is Capabilities 001–005 and Scenarios 001–009 plus 011. |
| `PKB-BL-007` | `FEATURE` | `PKB-MAPPING-001` | Carry scenario test observations through resolved production symbols into provider-neutral component identities, then expand only from production seeds to propose realization chains. | `NEEDS_RECONCILIATION` | Preserve PK-S1 v0.2 and select a new contract version after BL-006. Exclude test nodes from production components; distinguish direct links from Graphify-inferred expansion; fail closed on missing identity/evidence; report direct-symbol recall, expanded-chain coverage, and exact-component precision/recall separately. Feasibility evidence: the verified Java extractor produced 144 resolved production-symbol observations while preserving 891 unresolved references as gaps. |
