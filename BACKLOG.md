...
| `SF-BL-005` | `FEATURE` | `AUTH-002`, `PK-004`, `EVID-001`, `SF-EVAL-001`, `TECH-001`; source: SF-BL-002 post-acceptance analysis | Align scenario-mapping scoring units, strengthen evidence-backed METHOD realization chains, and compare baseline/improved producers on isolated unseen data under one frozen protocol. Keep TYPE limitations explicit and outputs proposal-only. | `IN_PROGRESS` | `SF-BL-005-ROUTE-COVERAGE-ANALYSIS-001` has exact read-only envelope `2f3181fddad7f5ee4cb160d781efd278fb2ed2b4`; producer preflight and a separately attributable exact-envelope preflight both passed, but analysis dispatch remains unauthorized. It covers 9 distinct observations accounting for 90 repeated `ROUTE_ABSENT` pairs. Generic follow-up candidate `8e835b427bd5f6b242b38d00e714d902298366c1` is `PARKED_AFTER_STAGE_2_FAIL`; it was not integrated and Stage 3 was not triggered. Findings are routed to `SF-BL-007` under CSI-001. REALWORLD-003 remains withheld; calibration, formal holdout and parent closure remain gated. |
...
| `SF-BL-007` | `FEATURE` | `CSI-001`, `AUTH-001`–`AUTH-003`, `PK-005`, `FD-T3-007`, `FD-T4-001`–`FD-T4-003`, `EVID-001`, `EXEC-001` | Provide a reusable, evidence-bound method for converting verified delivery findings into bounded system-improvement recommendations and routing authorized changes through existing Software Factory controls, without creating a parallel lifecycle or automatic Product truth. | `IN_PROGRESS` | Human selected `SF-BL-007-CONTINUOUS-SYSTEM-IMPROVEMENT-IMPLEMENTATION-001` from reviewed design candidate `f755640a8f7bef006b42f1dad792e723d21eb195`. Scope is the bounded Java validator/CLI, four canonical recommendation records, tests, evidence, and control synchronization. The recommendations themselves, deployment, automatic remediation, agent/automation changes, Product truth, and parent closure remain unauthorized. |
...
`SF-BL-005-ROUTE-COVERAGE-ANALYSIS-001` execution below. Human Authority has
selected it for envelope construction and preflight only. Its exact envelope
and producer preflight are present; independent preflight and dispatch remain
separate decisions.
...
`2f3181fddad7f5ee4cb160d781efd278fb2ed2b4` has producer preflight
`PASS_PENDING_INDEPENDENT_PREFLIGHT`; it cannot be dispatched until independent
preflight passes and Human Authority makes a separate dispatch decision.
