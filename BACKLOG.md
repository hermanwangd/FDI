...
| `SF-BL-005` | `FEATURE` | `AUTH-002`, `PK-004`, `EVID-001`, `TECH-001`; source: SF-BL-002 post-acceptance analysis | Align scenario-mapping scoring units, strengthen evidence-backed METHOD realization chains, and compare baseline/improved producers on isolated unseen data under one frozen protocol. Keep TYPE limitations explicit and outputs proposal-only. | `IN_PROGRESS` | RealWorld first cross-repository validation selected; ingress preparation in progress, frozen007 matching rules. Prior Petclinic007 recall70%, precision84.85% remains independently verified, not parent closure. Evidence: `validation/software-factory/sf-bl005/method-quality-007/RESULTS.md`; plan: `IMPLEMENTATION-PLAN.md#current-selection`. Overall complete-chain gate remains unavailable. Existing PORTABILITY-002 remediation retains its pinned scope. Formal holdout remains gated; no SF-BL-002 closure. |
