...
| `SF-BL-005` | `FEATURE` | `AUTH-002`, `PK-004`, `EVID-001`, `TECH-001`; source: SF-BL-002 post-acceptance analysis | Align scenario-mapping scoring units, strengthen evidence-backed METHOD realization chains, and compare baseline/improved producers on isolated unseen data under one frozen protocol. Keep TYPE limitations explicit and outputs proposal-only. | `IN_PROGRESS` | Current selection: bounded real-data Petclinic reproduction SF-BL-005-PETCLINIC-REPLAY-004, not successor METHOD scoring. Java evaluator increment independently reviewed; active plan: `IMPLEMENTATION-PLAN.md#current-selection`. Existing PORTABILITY-002 remediation retains its pinned scope. Feasibility receipt: `validation/software-factory/sf-bl005/acceptance-feasibility-001.md`. Formal holdout and production scoring remain gated; no SF-BL-002 closure. |
...
CLI/module-root correction, Java evaluator preparation and bounded real-data reproduction under
