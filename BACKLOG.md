...
| `SF-BL-005` | `FEATURE` | `AUTH-002`, `PK-004`, `EVID-001`, `SF-EVAL-001`, `TECH-001`; source: SF-BL-002 post-acceptance analysis | Align scenario-mapping scoring units, strengthen evidence-backed METHOD realization chains, and compare baseline/improved producers on isolated unseen data under one frozen protocol. Keep TYPE limitations explicit and outputs proposal-only. | `IN_PROGRESS` | `SF-BL-005-CROSSREPO-REALWORLD-002` selected: one revised calibration using reviewed containment candidate `ec604f1`; frozen scenarios/truth/algorithms, new immutable outputs, independent proof and receipt. First RealWorld no-score evidence remains immutable at `validation/software-factory/sf-bl005/cross-repo-realworld-001/RESULTS.md`; plan: `IMPLEMENTATION-PLAN.md#current-selection`. Petclinic007 metrics are not generalization or parent closure. Formal holdout remains gated. |
| `SF-BL-006` | `DOCUMENTATION` | `AUTH-001`, `AUTH-003`, `EXEC-003`, `EVID-001`; source: Human-authorized company-AI sharing review | Produce a bounded company-AI workflow learning package with physical learner/evaluator isolation, explicit sharing classification, deterministic file manifests, and no claim of runtime validation. | `BLOCKED_USER_APPROVAL` | Learner/evaluator archives and deterministic verification are complete at candidate `8661316a4f13098429ba61beb118a56cf5e66a6b`; evidence: `docs/company-ai-learning/RELEASE-VERIFICATION.json`. Upload remains blocked until company classification, recipient/environment and owner approval are bound. This lane is independent of calibration. |
...
their defined authority gates. `SF-BL-005` is selected for
`SF-BL-005-CROSSREPO-REALWORLD-002`, one revised cross-repository calibration
toward Human-selected recall >60% and precision >80%, with frozen RealWorld
inputs/gold/algorithms and immutable successor outputs under
