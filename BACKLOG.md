...
| `SF-BL-002` | `BUG` | `AUTH-002`, `PK-004`, `EVID-001`, `TECH-001`; source: immutable PKB-BL-007/011 `REVISE` evidence | Correct the misleading unresolved-reference denominator, recover only mechanically provable production receiver references, and enable evidence-backed scenario mapping without treating external/test-helper calls or proposals as Product truth. | `IN_PROGRESS` | Execution `SF-BL-002-PRODUCTION-SCENARIO-001` starts from exact control revision `f6ac9d0d389c6857ec0374575bd5af312b704ae9`. Use a new immutable run; do not mutate completed PKB-001 evidence or access evaluator-only mappings during generation. |
