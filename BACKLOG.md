...
| `PKB-BL-027` | `BUG` | `PKB-RUNTIME-001` | Make the frozen external Graphify runtime portable in the active workspace and harden Java stdio-MCP response and shutdown lifecycle. | `VERIFIED` | Candidate `a022b894ff2080390da87eeb017fa243f5afc1b7`: active-workspace Graphify 0.1.14 / MCP 1.29.1 is EXACTLY_BOUND with seven operations and clean close; Java adds configurable request timeout, malformed-response normalization, bounded forced shutdown, stream cleanup, and preserved interruption. Verification: 715 Java tests, 57 Python tests, public checks 9/9 and 24/24, live CLI PASS. Evidence: `validation/pkb001/runtime/pkb-bl027-portable-runtime-evidence.json`. Old runtime remains an untouched recovery source. Human Reviewer authorized terminal closure on 2026-09-06. |
...
Maturity for the bound spec revision: 24 normative requirements, 11 `VERIFIED`,
