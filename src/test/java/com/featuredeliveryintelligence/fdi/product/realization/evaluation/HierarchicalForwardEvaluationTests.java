...
class HierarchicalForwardEvaluationTests {
    ...
    @Test void positiveAndMixedMetricsKeepExactWeakerAndSupportingCreditSeparate() {
        ...
        assertEquals(2, report.component().sourcePathMatched());
        assertEquals(1, report.component().containingTypeMatched());
        assertEquals(1, report.component().bareSymbolOverlap());
        ...
        assertTrue(report.component().missing().contains(HierarchicalForwardEvaluation.Identity.from(expectedB)));
        assertTrue(report.component().extra().contains(HierarchicalForwardEvaluation.Identity.from(extra)));
    }
    ...
    private static HierarchicalForwardEvaluation.EvaluatorTruth truth(ObjectNode... ids) {
        return new HierarchicalForwardEvaluation.EvaluatorTruth("gold", Arrays.stream(ids).map(x -> new HierarchicalForwardEvaluation.Expected("ref", "node", HierarchicalForwardEvaluation.Identity.from(x))).toList());
    }
