...
/**
 * Deterministic code-baseline result. Wraps the arm-generator result object
 * and renders it byte-for-byte like the Python CLI's
 * {@code json.dumps(result, indent=2) + "\n"}: insertion order (no key
 * sorting), two-space indents, {@code ": "} key separator, default
 * {@code ensure_ascii=True} string escaping, and Python float rendering for
 * the confidence values.
 */
public record CodeBaselineResult(JsonNode result) {
    ...
    /** Python {@code repr(float)} for the finite confidence values this result uses. */
