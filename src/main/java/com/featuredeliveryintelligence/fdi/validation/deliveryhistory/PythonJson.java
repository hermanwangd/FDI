 ...
 * failure surface for its {@code --prs} input. Mirrors
 * {@code json.loads(Path.read_text())}: UTF-8 is decoded strictly (invalid
 * bytes raise the CPython codec message), content after the first value raises
 * {@code Extra data}, empty or whitespace-only input raises
 * {@code Expecting value}, {@code NaN}/{@code Infinity} are accepted like
 * CPython, and parse failures raise an {@link IllegalArgumentException} whose
 * message is byte-identical to the Python {@code json.JSONDecodeError} text,
 * including line, column, and character offset. Positions count Unicode code
 * points, like CPython.
 */
