...
public final class HierarchicalForwardEvaluationGenerator {
    ...
    private static HierarchicalForwardEvaluation.EvaluatorTruth truth(Path root){ProviderNeutralEvaluatorTruth t=ProviderNeutralEvaluatorTruth.load(root);return new HierarchicalForwardEvaluation.EvaluatorTruth(t.goldSha256(),t.mappings().stream().flatMap(m->m.expectedComponents().stream().map(e->new HierarchicalForwardEvaluation.Expected(m.capabilityId(),e.componentRef(),e.graphNodeId(),new HierarchicalForwardEvaluation.Identity(e.identity().canonicalRevision(),e.identity().sourcePath(),e.identity().granularity(),containing(e.identity()),e.identity().qualifiedSymbol())))).toList());}
    ...
    private static void write(Path p,byte[] b)throws Exception{Files.createDirectories(p.getParent());if(Files.exists(p)){require(Arrays.equals(Files.readAllBytes(p),b),"output collision with changed existing bytes: "+p);return;}Files.write(p,b,StandardOpenOption.CREATE_NEW);}
