...
| G4 — Previously unseen formal holdout | Holdout selected and sealed after the final candidate, scorer, thresholds, and selection protocol are frozen; no reuse for development/calibration; precision strictly >0.80 and recall strictly >0.60; all denominators and unavailable values explicit | Protocol revision 1 and a 473-entry negative exposure ledger are prepared and independently reviewed PASS. H1 gap review proves the legacy calibration scorer is incompatible; the first versioned-scorer envelope draft failed preflight and now requires exact vector/golden and verification-tool materialization. H0 and H1 remain unbound; H2 still needs Human disclosure attestation and an external seal. Selection, truth, executable envelope, dispatch, and execution remain missing. | **MISSING** |
