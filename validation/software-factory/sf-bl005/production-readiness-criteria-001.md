...
|---|---|---|---|
| G0 — Governance and reproducibility | Exact full revisions, frozen inputs and scoring policy; isolated producer/evaluator roles; immutable failures; deterministic manifests and two-run parity | Envelope and diagnostic controls exist; diagnostic 003 has two-run parity. BOXING-003 exposed calibration has a preserved command-evidence failure. | **INCOMPLETE** |
...
| G4 — Previously unseen formal holdout | Holdout selected and sealed before final candidate; no reuse for development/calibration; frozen scorer; precision strictly >0.80 and recall strictly >0.60; all denominators and unavailable values explicit | No authorized, sealed, previously unseen formal holdout execution exists. | **MISSING** |
| G5 — Operational readiness | Repeatable execution with durable command/attempt/exit/order evidence; input/output digests; failure recovery; bounded runtime; audit trail and rollback/runbook | BOXING-003 demonstrates a command-evidence gap. No production runbook or rollback exercise is bound. | **INCOMPLETE** |
...
4. Freeze the final candidate, scorer, thresholds, and holdout-selection protocol before choosing the unseen repositories.
