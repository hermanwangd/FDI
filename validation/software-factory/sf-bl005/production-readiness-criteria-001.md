...
| G4 — Previously unseen formal holdout | Holdout selected and sealed after the final candidate, scorer, thresholds, and selection protocol are frozen; no reuse for development/calibration; precision strictly >0.80 and recall strictly >0.60; all denominators and unavailable values explicit | Protocol revision 1 and a 473-entry negative exposure ledger are prepared and independently reviewed PASS. H0 and H1 remain unbound; H2 still needs Human disclosure attestation and an external seal. Selection, truth, execution envelope, dispatch, and execution remain missing. | **MISSING** |
