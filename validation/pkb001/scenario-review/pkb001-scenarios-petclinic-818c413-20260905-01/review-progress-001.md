...
| HYP-CAPABILITY-003 | 飼主名下寵物資料與同名限制 | ACCEPT |
| HYP-SCENARIO-005 | 在一位飼主名下建立寵物資料 | ACCEPT |
| HYP-SCENARIO-006 | 拒絕同一飼主名下的重複寵物名稱 | ACCEPT |

對應產品語意已保存為 `accepted-semantics-003.json`，包含三項能力與六個已接受情境。
原始 proposal.json、review.json 與 review.md 保持原樣。新增的
`review-decisions-003.json` 累計記錄九個接受決定；原始提案尚有 7 個決策空白。
另有自動補充的 `HYP-SCENARIO-011` 保持 `PROPOSAL_ONLY`，因此合計 8 個待審決定。

本次接受是前三組實驗輸入的審核與凍結，不代表其他能力或補充情境已接受，也不是整體 GO。
...
`acceptance-manifest-003.json`；補充提案及其限制見
`supplemental-scenario-proposal-001.json`。
