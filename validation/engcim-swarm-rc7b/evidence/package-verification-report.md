...
> 本檔由 `verify.sh` 自動生成（20260919-175906-22966）。重新執行 `bash verify.sh` 會覆寫本檔。
...
- 結果統計：PASS=27 / FAIL=0 / WAIVED=2 / NOT VERIFIED=2（required=0、optional-external=2）
...
- 暫存資源：SMOKE-TEST-20260919-175906-22966（已於結尾標 cancelled 清理）

## Smoke test 明細（27 項 required + optional-external 附項）
...
|---|---|---|---|---|---|---|
| 1. Skills 存在 | required | `multica skill list --output json` | 31 個 skill：artifact-consistency changesurface-analysis cicd-automation code-review-method deployment-verification execution-guard human-input-resume incident-response multica-cli observability-slo performance-benchmark pk-azure-devops-history pk-correlation-synthesis pk-document-analysis pk-file-ing | 全部存在 | PASS | skill list 含全部 31 個名稱 |
...
| 4. Squad 存在 | required | `multica squad list --output json` | squad「Swarm」存在 | 存在 (983228bf-005a-4049-ae56-2893ac76c227) | PASS | squad list 含名稱 Swarm |
| 5. Squad leader 正確 | required | `multica squad list --output json` | leader = Swarm Orchestrator | leader=69a4494a-e267-4efe-af37-959482049935 | PASS | leader 欄位匹配 69a4494a-e267-4efe-af37-959482049935 |
| 6. Squad 成員 18 名 | required | `multica squad member list 983228bf-005a-4049-ae56-2893ac76c227` | 18 名成員（leader 以外全員） | member list 共 19 筆，18 名預期成員全在 | PASS | 逐一比對 agent id 通過 |
| 7. ProductKB project 存在 | required | `multica project list --output json` | project「ProductKB」存在 | 存在 (d4a3c74f-f8cf-47ac-b712-766d1ee16a3e) | PASS | project list 含 title ProductKB |
...
| 9. 建暫存 issue | required | `multica issue create --title SMOKE-TEST-20260919-175906-22966 --description-stdin --project d4a3c74f-f8cf-47ac-b712-766d1ee16a3e` | 建立成功並回傳 id | id=01a0b91b-6ae1-711c-822d-c578947fed2c | PASS | issue create 回傳有效 id |
| 10. 加 comment | required | `multica issue comment add 01a0b91b-6ae1-711c-822d-c578947fed2c --content ...` | exit 0 | exit 0 | PASS | comment add 成功 |
| 11. Metadata 寫讀 | required | `issue metadata set --key smoke_key --value 20260919-175906-22966；get --key smoke_key` | 讀回 = 20260919-175906-22966 | 讀回 = "20260919-175906-22966" | PASS | 寫讀一致（upsert 冪等） |
| 12. Label 貼讀 | required | `issue label add + issue get` | issue get 輸出含 label id 5ddaa77c-b605-4318-9871-b70afd66af2e | 貼上並讀回成功 | PASS | issue get 含 5ddaa77c-b605-4318-9871-b70afd66af2e |
| 13. Assign squad 可執行 | required | `multica issue assign 01a0b91b-6ae1-711c-822d-c578947fed2c --to Swarm --no-start` | exit 0（--no-start 不觸發 run） | exit 0 | PASS | assign 指令成功 |
| 14. Swarm compact stateRef metadata 讀寫 | required | `issue metadata set/get swarm.state*` | stateRevision=2、summaryStatus=RUNNING | 讀回一致 | PASS | compact stateRef projection usable; no legacy expanded child metadata keys required |
| 15. issue children 可查 | required | `multica issue children 01a0b91b-6ae1-711c-822d-c578947fed2c --output json` | 列出子 issue 含 01a0b91b-96f3-75d6-a671-9908420a90df | 列出且含子 issue id | PASS | issue children 輸出含 01a0b91b-96f3-75d6-a671-9908420a90df |
| 16. 重複建立防護 | required | `multica issue create --title SMOKE-TEST-20260919-175906-22966（同名）` | 失敗且回 409 active_duplicate_issue | 拒絕重複建立 | PASS | 同名活躍 issue 被拒：Active duplicate issue exists: E7B-4 SMOKE-TEST-20260919-175906-22966 (status: todo). Set allow_duplicate=true or use --allow-duplicate to create another. |
...
| 19. pk/ store schema 驗證 | required | `python3 pk/_schema/validate_store.py --root pk` | exit 0（全部通過） | OK: 17 個條目／記錄／node／edge 全部通過（root=/Users/herman_mbp2023/engcim-swarm-rc7a1-validation-20260919/engcim-swarm-package-RC7A1/pk） | PASS | OK: 17 個條目／記錄／node／edge 全部通過（root=/Users/herman_mbp2023/engcim-swarm-rc7a1-validation-20260919/engcim-swarm-package-RC7A1/pk） |
| 20. Graphify-level analyzer self-test | required | `bash skills/pk-repository-analysis/scripts/self-test.sh` | exit 0 + cross-repo CALLS | PASS graphify-level self-test: 13 nodes / 18 edges | PASS | [graphify-analyzer] chart-management-api@unknown: 11 observations -> /var/folders/f7/cxmy128d6mq88dkh76zm1v3r0000gn/T/tmp.BfEStgraBH/out/api.jsonl [graphify-analyzer] chart-viewer@unknown: 7 observations -> /var/folders/f7/cxmy128d6mq88dkh76zm1v3r0000gn/T/tmp.BfEStgraBH/out/viewer.jsonl [correlate]  |
...
| 22. RC5 autonomous fan-in contract | required | `static contract` | child in_review + structured parent event + execution-state ALL_REQUIRED | contract present | PASS | human child done is not required for fan-in; native stage is optional wake-up only |
| 23. Runtime PK support bundles | required | `static contract` | pk-file-ingestion + verification-protocol carry runtime-pk baseline | bundles present | PASS | ephemeral workdirs can hydrate task-scoped pk/ and validate independently |
...
| 附項 B. TKMS MCP server 連線 | optional-external | `（需人工驗證）` | TKMS MCP 可查詢 | 自動化無法驗證 | WAIVED | 支援 runtime_native / auto / multica_managed；真實 MCP connectivity 需在目標 runtime 驗證（waive 理由：WAIVE 環境變數明確豁免） |
| 附項 C. Azure DevOps MCP server 連線 | optional-external | `（需人工驗證）` | Azure DevOps MCP 可查詢 | 自動化無法驗證 | WAIVED | 同上；MCP 註冊為選配（waive 理由：WAIVE 環境變數明確豁免） |
...
| 24. RC6 curated engineering skill pack self-test | required | `bash skills/rc6-self-test.sh` | exit 0 | PASS | PASS | PASS intention spec ALLOW path=/private/var/folders/f7/cxmy128d6mq88dkh76zm1v3r0000gn/T/tmp.sfN5I8OfEG/ok.txt root=/private/var/folders/f7/cxmy128d6mq88dkh76zm1v3r0000gn/T/tmp.sfN5I8OfEG DENY recursive delete of root ALLOW PASS artifact trace PASS {"name": "latency_p95_ms", "baseline": 250, "current |
| 25. RC6 critical agent-skill bindings | required | `static frontmatter contract` | critical roles declare RC6 skills | bindings present | PASS | PM/Coder/QA/Release/Performance bindings verified |
| 26. RC7-A.1 human input/resume self-test | required | `bash skills/rc7a-self-test.sh` | exit 0 | PASS | PASS | PASS QuestionSet QS-E6B-S04-001 PASS DecisionResponse DEC-E6B-S04-001 PASS MissionResumeEvent RESUME-E6B-S04-001 PASS init /var/folders/f7/cxmy128d6mq88dkh76zm1v3r0000gn/T/tmp.wnK7j8lAyv/.swarm/missions/M-S04-DEMO/state.json PASS waiting_for_input QS-E6B-S04-001 revision 2 STALE_STATE expected=1 cur |
| 27. RC7-A.1 critical skill bindings | required | `static frontmatter contract` | orchestrator/product-manager/reviewer/verifier declare human-input-resume | bindings present | PASS | human-input-resume bindings verified |
...
| TKMS MCP 整合 | WAIVED | runtime-native / auto / Multica-managed supported；真實 capability 需目標 runtime 驗證 |
| Azure DevOps MCP 整合 | WAIVED | runtime-native / auto / Multica-managed supported；真實 capability 需目標 runtime 驗證 |
...
3. 檢查 dispatch acknowledgement：讀 parent issue metadata `legacy expanded child metadata.dispatchStatus`，
