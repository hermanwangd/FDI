...
3. [Approved-source lock](governance/approved-source-lock.json)
4. [Governing-source index](governance/GOVERNING-SOURCES.md)
5. [Current readiness status](STATUS.json)
...
- Keep claims within the evidence actually executed. Local deterministic tests do not prove live provider binding, real Product binding, DEV-204 execution, F001 effectiveness, upstream byte identity, or production readiness. Check [STATUS.json](STATUS.json) and [VERIFICATION-SUMMARY.json](VERIFICATION-SUMMARY.json) before making completion claims.
