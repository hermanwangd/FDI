...
**使用者不需要先理解 19 個角色**——只要用場景前綴（`[S01]`–`[S10]`）開 issue 指派給 Swarm squad，Orchestrator 會依 [`docs/scenarios.md`](docs/scenarios.md) 的 playbook 做動態選才與角色編排。三大類、10 個場景：
...
| skill 漸進載入 | `multica skill` + SKILL.md | 20 個 skill（編排、swarm 自觀測遙測、CLI 速查、產品知識庫操作、6 個 pk-* ingestion、變更影響分析、測試設計、事故指揮、SLO、發布管理等領域 skill，見 `skills/`）；掛載對應宣告在各 agent frontmatter 的 `skills:` 欄位，setup.sh 依宣告用 `multica agent skills add` 掛載 |
