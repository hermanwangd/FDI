# Multica Swarm Verification Report（環境：stub）

> ⚠️⚠️⚠️ **本報告在 stub 環境產出**（`multica version` 輸出含 stub 標記）：所有 PASS 僅代表 stub 行為，**不得作為真實部署的驗證依據**。請在真實 Multica 環境重跑 `bash verify.sh` 後再引用本報告。 ⚠️⚠️⚠️

> 本檔由 `verify.sh` 自動生成（20260919-015824-14735）。重新執行 `bash verify.sh` 會覆寫本檔。
> 本檔定位為 **verification report（環境：stub）**，不是部署完成證明。

- multica CLI：multica 0.5.0 (stub)
- 執行環境：**stub**（依 `multica version` 輸出判定）
- 查證基準：multica CLI v0.5.0 / repo HEAD 2df765a（見 docs/multica-cli-verification.md）
- 結果統計：PASS=19 / FAIL=0 / WAIVED=0 / NOT VERIFIED=4（required=0、optional-external=4）
...
- 暫存資源：SMOKE-TEST-20260919-015824-14735（已於結尾標 cancelled 清理）

## Smoke test 明細（19 項 required + optional-external 附項）
...
schema 驗證（本地確定性，跑 pk/_schema/validate_store.py）。swarm 行為項
...
| 4. Squad 存在 | required | `multica squad list --output json` | squad「Swarm」存在 | 存在 (29165cb3-2c42-4c7e-bfeb-935fcb88669f) | PASS | squad list 含名稱 Swarm |
| 5. Squad leader 正確 | required | `multica squad list --output json` | leader = Swarm Orchestrator | leader=e6feeb1f-9790-4859-879a-d932f81e4fe1 | PASS | leader 欄位匹配 e6feeb1f-9790-4859-879a-d932f81e4fe1 |
| 6. Squad 成員 18 名 | required | `multica squad member list 29165cb3-2c42-4c7e-bfeb-935fcb88669f` | 18 名成員（leader 以外全員） | member list 共 18 筆，18 名預期成員全在 | PASS | 逐一比對 agent id 通過 |
| 7. ProductKB project 存在 | required | `multica project list --output json` | project「ProductKB」存在 | 存在 (9be99f92-8f81-4c40-85fa-432a244f43fe) | PASS | project list 含 title ProductKB |
| 8. 10 個 label 存在 | required | `multica label list --resource-type issue --output json` | pk-semantics pk-architecture pk-realization pk-governance src-team-seed src-product-docs src-test-assets src-operations src-engineering src-code-delivery | 全部存在 | PASS | label list 含全部 10 個名稱 |
| 9. 建暫存 issue | required | `multica issue create --title SMOKE-TEST-20260919-015824-14735 --description-stdin --project 9be99f92-8f81-4c40-85fa-432a244f43fe` | 建立成功並回傳 id | id=39007489-4a40-451e-9da1-5d6b4e1ebfeb | PASS | issue create 回傳有效 id |
| 10. 加 comment | required | `multica issue comment add 39007489-4a40-451e-9da1-5d6b4e1ebfeb --content ...` | exit 0 | exit 0 | PASS | comment add 成功 |
| 11. Metadata 寫讀 | required | `issue metadata set --key smoke_key --value 20260919-015824-14735；get --key smoke_key` | 讀回 = 20260919-015824-14735 | 讀回 = 20260919-015824-14735 | PASS | 寫讀一致（upsert 冪等） |
| 12. Label 貼讀 | required | `issue label add + issue get` | issue get 輸出含 label id fd9d2304-47ed-44ac-bd32-bce971b5cce2 | 貼上並讀回成功 | PASS | issue get 含 fd9d2304-47ed-44ac-bd32-bce971b5cce2 |
| 13. Assign squad 可執行 | required | `multica issue assign 39007489-4a40-451e-9da1-5d6b4e1ebfeb --to Swarm --no-start` | exit 0（--no-start 不觸發 run） | exit 0 | PASS | assign 指令成功 |
...
| 15. issue children 可查 | required | `multica issue children 39007489-4a40-451e-9da1-5d6b4e1ebfeb` | 列出子 issue 含 c0eaac57-bb49-49d8-a909-e76025c42981 | 列出且含子 issue id | PASS | issue children 輸出含 c0eaac57-bb49-49d8-a909-e76025c42981 |
| 16. 重複建立防護 | required | `multica issue create --title SMOKE-TEST-20260919-015824-14735（同名）` | 失敗且回 409 active_duplicate_issue | 拒絕重複建立 | PASS | 同名活躍 issue 被拒：409 active_duplicate_issue: an active issue with this title exists |
...
| 19. pk/ store schema 驗證 | required | `python3 pk/_schema/validate_store.py --root pk` | exit 0（全部通過） | OK: 17 個條目／記錄／node／edge 全部通過（root=/mnt/agents/output/multica-swarm/pk） | PASS | OK: 17 個條目／記錄／node／edge 全部通過（root=/mnt/agents/output/multica-swarm/pk） |
...
| 附項 B. TKMS MCP server 連線 | optional-external | `（需人工驗證）` | TKMS MCP 可查詢 | 自動化無法驗證 | NOT VERIFIED | MCP 註冊為兩段式（workspace mcp add + agent mcp add），見 setup.sh 的 MCP OPTIONAL-SKIP 說明與 docs/multica-cli-verification.md |
...
|---|---|---|
| Multica CLI 可用 | PASS | multica 0.5.0 (stub)（環境：stub）；preflight auth 檢查通過 |
...
| TKMS MCP 整合 | NOT VERIFIED | MCP server 註冊為兩段式（workspace mcp add + agent mcp add）選配；見 setup.sh OPTIONAL-SKIP 說明與附項 B |
| Azure DevOps MCP 整合 | NOT VERIFIED | 同上；見附項 C |
| Multi-repo analysis | NOT VERIFIED | 需真實 repo 端到端驗證 |
| Code Graph synthesis | NOT VERIFIED | 需真實 code graph 端到端驗證 |
