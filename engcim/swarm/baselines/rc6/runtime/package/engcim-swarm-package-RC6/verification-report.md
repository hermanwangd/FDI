...
> 本檔由 `verify.sh` 自動生成（20260926-092726-26259）。重新執行 `bash verify.sh` 會覆寫本檔。
> 本檔定位為 **verification report（環境：real）**，不是部署完成證明。

- multica CLI：multica 0.5.3 (commit: ff8b28549, built: 2026-09-24T10:07:17Z)
- 執行環境：**real**（依 `multica version` 輸出判定）
- 查證基準：multica CLI v0.5.0 / repo HEAD 2df765a（見 docs/multica-cli-verification.md）
- 結果統計：PASS=26 / FAIL=0 / WAIVED=4 / NOT VERIFIED=0（required=0、optional-external=0）
...
- 暫存資源：SMOKE-TEST-20260926-092726-26259（已於結尾標 cancelled 清理）
...
| 4. Squad 存在 | required | `multica squad list --output json` | squad「Swarm」存在 | 存在 (e6db7748-9706-42c9-ac87-78e248833f59) | PASS | squad list 含名稱 Swarm |
| 5. Squad leader 正確 | required | `multica squad list --output json` | leader = Swarm Orchestrator | leader=770fc0b2-e0c1-49dc-af0f-a93ecb1407f3 | PASS | leader 欄位匹配 770fc0b2-e0c1-49dc-af0f-a93ecb1407f3 |
| 6. Squad 成員 18 名 | required | `multica squad member list e6db7748-9706-42c9-ac87-78e248833f59` | 18 名成員（leader 以外全員） | member list 共 19 筆，18 名預期成員全在 | PASS | 逐一比對 agent id 通過 |
| 7. ProductKB project 存在 | required | `multica project list --output json` | project「ProductKB」存在 | 存在 (16237cba-512e-4d59-8eef-80174649c098) | PASS | project list 含 title ProductKB |
...
| 9. 建暫存 issue | required | `multica issue create --title SMOKE-TEST-20260926-092726-26259 --description-stdin --project 16237cba-512e-4d59-8eef-80174649c098` | 建立成功並回傳 id | id=01a0db53-8048-7659-8b0c-1893afaf1389 | PASS | issue create 回傳有效 id |
| 10. 加 comment | required | `multica issue comment add 01a0db53-8048-7659-8b0c-1893afaf1389 --content ...` | exit 0 | exit 0 | PASS | comment add 成功 |
| 11. Metadata 寫讀 | required | `issue metadata set --key smoke_key --value 20260926-092726-26259；get --key smoke_key` | 讀回 = 20260926-092726-26259 | 讀回 = "20260926-092726-26259" | PASS | 寫讀一致（upsert 冪等） |
| 12. Label 貼讀 | required | `issue label add + issue get` | issue get 輸出含 label id 8533dbb9-ba24-4554-8d0a-2ccf9e20a874 | 貼上並讀回成功 | PASS | issue get 含 8533dbb9-ba24-4554-8d0a-2ccf9e20a874 |
| 13. Assign squad 可執行 | required | `multica issue assign 01a0db53-8048-7659-8b0c-1893afaf1389 --to Swarm --no-start` | exit 0（--no-start 不觸發 run） | exit 0 | PASS | assign 指令成功 |
...
| 15. issue children 可查 | required | `multica issue children 01a0db53-8048-7659-8b0c-1893afaf1389 --output json` | 列出子 issue 含 01a0db53-a995-7b9b-856e-540b459d131f | 列出且含子 issue id | PASS | issue children 輸出含 01a0db53-a995-7b9b-856e-540b459d131f |
| 16. 重複建立防護 | required | `multica issue create --title SMOKE-TEST-20260926-092726-26259（同名）` | 失敗且回 409 active_duplicate_issue | 拒絕重複建立 | PASS | 同名活躍 issue 被拒：Active duplicate issue exists: E6V-74 SMOKE-TEST-20260926-092726-26259 (status: todo). Set allow_duplicate=true or use --allow-duplicate to create another. |
...
| 17b. Scenario playbook runtime resource | required | `test -f skills/scenario-playbooks/SKILL.md；orchestrator frontmatter` | scenario-playbooks 含 S01–S10 且 Orchestrator 宣告掛載 | runtime skill 存在且已宣告 | PASS | full playbook is materialized as a skill; setup.sh dynamic scan + frontmatter attach make it agent-readable |
...
| 19. pk/ store schema 驗證 | required | `python3 pk/_schema/validate_store.py --root pk` | exit 0（全部通過） | OK: 17 個條目／記錄／node／edge 全部通過（root=/Users/herman_mbp2023/Documents/Feature-Delivery-Intelligence/engcim/skill-packs/rc6-runtime-baseline-v1/package/engcim-swarm-package-RC6/pk） | PASS | OK: 17 個條目／記錄／node／edge 全部通過（root=/Users/herman_mbp2023/Documents/Feature-Delivery-Intelligence/engcim/skill-packs/rc6-runtime-baseline-v1/package/engcim-swarm-package-RC6/pk） |
| 20. Graphify-level analyzer self-test | required | `bash skills/pk-repository-analysis/scripts/self-test.sh` | exit 0 + cross-repo CALLS | PASS graphify-level self-test: 13 nodes / 18 edges | PASS | [graphify-analyzer] chart-management-api@unknown: 11 observations -> /var/folders/f7/cxmy128d6mq88dkh76zm1v3r0000gn/T/tmp.cfqs9dlgFU/out/api.jsonl [graphify-analyzer] chart-viewer@unknown: 7 observations -> /var/folders/f7/cxmy128d6mq88dkh76zm1v3r0000gn/T/tmp.cfqs9dlgFU/out/viewer.jsonl [correlate]  |
...
| 附項 A. Agent 實際 run 行為 | optional-external | `（需人工觀察）` | 指派真實任務後 squad 協作正常 | 自動化無法驗證 | WAIVED | 請指派真實任務並觀察 examples/first-mission.md 流程；執行手冊見報告 Z20 章節（waive 理由：WAIVE 環境變數明確豁免） |
...
| 附項 D. Dispatch ack 可觀察（真實 agent） | optional-external | `multica issue runs <id>（以 --help 為準）` | 真實派工後可觀察 run queued/dispatched/running | 自動化無法驗證（無真實 agent run） | WAIVED | 見報告 Z20 手冊步驟 3（waive 理由：WAIVE 環境變數明確豁免） |
| 24. RC6 curated engineering skill pack self-test | required | `bash skills/rc6-self-test.sh` | exit 0 | PASS | PASS | PASS intention spec ALLOW path=/private/var/folders/f7/cxmy128d6mq88dkh76zm1v3r0000gn/T/tmp.rvJeL4klyn/ok.txt root=/private/var/folders/f7/cxmy128d6mq88dkh76zm1v3r0000gn/T/tmp.rvJeL4klyn DENY recursive delete of root ALLOW PASS artifact trace PASS {"name": "latency_p95_ms", "baseline": 250, "current |
...
|---|---|---|
| Multica CLI 可用 | PASS | multica 0.5.3 (commit: ff8b28549, built: 2026-09-24T10:07:17Z)（環境：real）；preflight auth 檢查通過 |
