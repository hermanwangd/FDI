...
class Rc6CompatibilityTests {
    ...
    void scenarioFirstRc6SurfaceAndCoreSkillsRemainPresent() throws Exception {
        Path root = Path.of("");
        Path scenarioDoc = root.resolve("baselines/rc6/runtime/package/engcim-swarm-package-RC6/docs/scenarios.md");
        ...
        for (String skill : new String[] {"swarm-orchestration", "execution-guard", "product-knowledge", "verification-protocol"}) {
            assertThat(Files.exists(root.resolve("baselines/rc6/skill-pack/skills/" + skill + "/SKILL.md")))
                    .as("RC6 skill %s", skill).isTrue();
        }
        Path fullPackage = root.resolve("baselines/rc6/runtime/package/engcim-swarm-package-RC6/skills");
